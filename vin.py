    ...
    def populate_buy_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        for i in self.buy_lookback_range:
            ...
            buy_conditions = [
                ...
                # (df[f"close_corr_{j}"] - df[f"close_corr_{i}"]).gt(0)
            ]
...
class ViNSellCorrV1(ViN):
    def custom_sell(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float,
        ...
        if trade_len <= 2:
            if current_profit < -0.08 and not candle_1['green']:
        ...
        if trade_len < self.sideways_candles and -0.015 < current_profit < 0.015:
            return None
        if current_profit < -0.16 and candle_1['streak_s_max'] < 1:
        ...
        if candle_1['streak_b'] >= candle_1['streak_s_min']:
            return None
        ...
        if current_profit < -0.04:
            ...
            offset = (sqrt(abs(current_profit * 100)) - i / (abs(current_profit * 100) + 0.01)) / 7200
        if close_corr_i_diff > offset and close_corr_ij_diff > 0:
        ...
        elif ef['vwrs_sell'].iat[-1] > 82 + current_profit and vwrs_corr_diff < 0:
            log.info(f"custom_sell: vwrs sell for pair {pair} with profit/loss {current_profit:.2f} and trade len {trade_len} on candle {df_trade['date'].iat[-1]}.")
            return f"vwrs sell ({buy_signals})"
        else:
            return None
