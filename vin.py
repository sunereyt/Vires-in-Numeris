...
class ViN(IStrategy):
    ...
    lookback_candles = 15
    indicator_range = range(2, buy_time_period + 1)
    ...
    def populate_sell_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        df.loc[:, 'sell_tag'] = ''
        time_periods = range(3, self.buy_time_period + 1)
    ...
    def custom_sell(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float,
        ...
        if current_profit > 0.015 and 'sell' in sell_tag:
            if candle_1['buy']:
                log.info(f"custom_sell: sell for pair {pair} with profit {round(current_profit, 2)}, sell_tag {sell_tag} and buy_tag {buy_tag} on candle {candle_1['date']} is cancelled because of buy signal {candle_1['buy_tag']}.")
                return None
            else:
