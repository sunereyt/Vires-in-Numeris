...
class ViNBuyPct(ViN):
    ...
    def populate_buy_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        df.loc[:, 'buy'] = df['buy_tags_sum'].ge(25)
...
class ViNSellCorr(ViN):
    ...
    def custom_sell(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float,
        ...
        if trade_len <= 2 or candle_1['buy'] or candle_1['streak_b'] >= candle_1['streak_s_min'] or (trade_len < self.lookback_candles and -0.04 < current_profit < 0.04) or abs(candle_1['streak_s_min']) <= 1:
            return None
